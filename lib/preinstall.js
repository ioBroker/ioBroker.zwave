    ...
    if (usepkgconfig) {
      ...
      if (configs[item].searchpattern) {
        ...
        for (var i in configs[item].locations) {
          ...
          var cmdfmt = "find %s -name '%s' -exec grep -H '%s' '{}' \\; 2> >(grep -v 'Permission denied' >&2)";
      ...
      for (var i in configs[item].locations) {
        ...
        if (configs[item].searchpattern) {
          ...
          cmdfmt = "find %s -name '%s' -exec grep -H '%s' '{}' \\; 2> >(grep -v 'Permission denied' >&2)";
          cmd = util.format(cmdfmt, loc, fp, sp);
          var found = doScript(cmd, true);
          if (!found) continue;
        }
...
function installOpenZwaveFromSource() {
  const version = 'openzwave-1.6.918';

...
function installOpenZwaveFromSource() {
  // For now we manually have to install 1.5 because openzwave-shared is not compatible yet
  const branch = '1.5';
  doScript(`curl -L -O https://github.com/OpenZWave/open-zwave/archive/V${branch}.zip`);
  doScript(`rm -rf open-zwave-${branch}`);
  doScript(`unzip open-zwave-${branch}.zip && rm open-zwave-${branch}.zip`);
  try {
